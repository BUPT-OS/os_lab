...
我们规定size太小的块，比如规定为2^8Bytes，统一放到f=0的blocks[0]\[s]中。

第一层索引f是按2的幂进行划分，也就是说，size值最高位的1是第多少位，f就是多少 - 上面规定的8 + 1。这样内存就被划分为了f=0时[2^8,2^9-1],f=1时[2^9,2^10-1]....。
...
$$
f = 0, s = 200 / (256/32) = (32/256)*200 = 25
$$
例如，当SLI=4, size=460时
$$
